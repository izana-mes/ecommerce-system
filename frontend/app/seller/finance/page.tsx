  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [balRes, txRes] = await Promise.all([
        fetch("/api/v1/seller/finance/balance", { headers: { Authorization: `Bearer ${token}` } }),
        fetch("/api/v1/seller/finance/transactions?size=50", { headers: { Authorization: `Bearer ${token}` } })
      ]);
