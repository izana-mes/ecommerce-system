...
export default function SellerReviewsPage() {
  ...
  const loadProducts = useCallback(async () => {
    ...
    try {
      const response = await fetch("/api/v1/seller/inventory", {
  ...
  const loadReviews = useCallback(async () => {
    ...
    try {
      const response = await fetch(`/api/v1/seller/reviews/${selectedProductId}?limit=50`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (response.ok) {
        const res = await response.json();
  ...
  const handleReply = async (reviewId: string) => {
    ...
    try {
      const response = await fetch(`/api/v1/seller/reviews/${selectedProductId}/${reviewId}/reply`, {
        method: "POST",
        headers: {
