...
export default function SellerCatalogPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        const ok =
          String(profile?.role || "").toLowerCase() === "seller" ||
          (Array.isArray(profile?.roles) && profile.roles.includes("ROLE_SELLER"));
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const response = await fetch("/api/products/seller/mine", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        cache: "no-store",
  ...
  if (!allowed)
    return <main style={{ padding: "2rem" }}>Seller role required.</main>;

  return (
    <main style={{ padding: "2rem", maxWidth: 720 }}>
      <h1 style={{ fontSize: "1.35rem", marginBottom: "0.5rem" }}>Your seller SKUs</h1>
      <p style={{ color: "#555", marginBottom: "1rem" }}>
        Products linked to your account after an approved seller create-request.
      </p>
